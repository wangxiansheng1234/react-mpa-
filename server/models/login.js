...
let renderFullPage = (html, initData) => {
...
<link href=${filePath.styles.login} rel="stylesheet" >
<script>
window.__INITIAL_STATE__ = ${JSON.stringify(initData)}
</script>
</head>
<body>
...
<script src=${filePath.javascript.login} type="text/javascript"></script>
</html>
    `
};
...
let login = (req, res) => {
    ...
    promiseAll([params])
        .then((data) => {
            let html = renderToString(<Login initData={data[0].response} />);
            res.end(renderFullPage(html, data[0].response));
            //清除内存
            html = null;
            params = null;
        })
